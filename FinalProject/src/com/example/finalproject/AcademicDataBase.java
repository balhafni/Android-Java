...
public class AcademicDataBase extends SQLiteOpenHelper {
	...
	public boolean insertData(String major, String className, String classTime, int credit, int classPriority) {
		...
		values.put(CLASS_CREDIT, credit);
	...
	public ArrayList<AcademicPlans> getClassesWithMajor(String major) {
		SQLiteDatabase db = this.getWritableDatabase();
		ArrayList<AcademicPlans> list = new ArrayList<AcademicPlans>();
		Cursor res = db.rawQuery("select * from " + TABLE_NAME + " where Major ='" + major + "'", null);
		while (res.moveToNext()) {
			list.add(new AcademicPlans(res.getString(0), res.getString(1), res.getString(2), res.getInt(3),
					res.getInt(4)));
		}
